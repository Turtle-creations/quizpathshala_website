...
class Database:
    ...
    def initialize(self):
        ...
        with self.connection() as conn:
            if self.is_sqlite:
                ...
                self._ensure_exam_hierarchy_tables(conn)
                ...
                self._ensure_column(conn, "payment_orders", "payment_status", "TEXT NOT NULL DEFAULT 'pending'")
                self._ensure_column(conn, "payment_orders", "premium_result", "TEXT")
            ...
            self._ensure_column(conn, "password_reset_requests", "password_reset_at", "TEXT")
            self._ensure_column(conn, "telegram_account_links", "telegram_first_name", "TEXT")
            self._ensure_user_activity_logs_table(conn)
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_sub_exams_category_id ON sub_exams(category_id, name)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_user_activity_logs_user_created ON user_activity_logs(user_id, created_at DESC)"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_exams_sub_exam_id ON exams(sub_exam_id) WHERE sub_exam_id IS NOT NULL"
            )
            conn.execute(
    ...
    def _initialize_postgres(self, conn):
        statements = [
            ...
            CREATE TABLE IF NOT EXISTS sub_exams (
                sub_exam_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                name TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS exams (
                ...
                sub_exam_id INTEGER UNIQUE REFERENCES sub_exams(sub_exam_id) ON DELETE CASCADE,
                description TEXT,
                created_at TEXT NOT NULL
            )
            """,
    ...
    def _ensure_user_activity_logs_table(self, conn):
        if self.is_postgres:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS user_activity_logs (
    ...
    def _sqlite_schema(self) -> str:
        ...
        CREATE TABLE IF NOT EXISTS sub_exams (
            sub_exam_id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS exams (
            ...
            title TEXT NOT NULL UNIQUE,
            sub_exam_id INTEGER UNIQUE,
            description TEXT,
            created_at TEXT NOT NULL,
            FOREIGN KEY (sub_exam_id) REFERENCES sub_exams(sub_exam_id) ON DELETE CASCADE
        );
    ...
    def _ensure_exam_hierarchy_tables(self, conn):
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS categories (
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS sub_exams (
                sub_exam_id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                category_id INTEGER NOT NULL,
                UNIQUE (category_id, name),
                FOREIGN KEY (category_id) REFERENCES categories(category_id) ON DELETE CASCADE
        ...
        self._ensure_column(conn, "exams", "sub_exam_id", "INTEGER")
        self._backfill_exam_hierarchy(conn)

    def _backfill_exam_hierarchy(self, conn):
        unmapped_rows = conn.execute(
            """
            SELECT exam_id, title
            FROM exams
            WHERE COALESCE(sub_exam_id, 0) = 0
            ORDER BY exam_id
            """
        ).fetchall()
        ...
        for row in unmapped_rows:
            ...
            conn.execute(
                "UPDATE exams SET sub_exam_id = ? WHERE exam_id = ?",
                (sub_exam_id, row["exam_id"]),
            )
